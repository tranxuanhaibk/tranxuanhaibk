...
public class ContactDAO extends AbstractDAO {
	...
	private static final String SQL_ADD = "INSERT INTO vnecontact(fullname, email, subject, content) VALUES(?, ?, ?, ?)";
	private static final String FIND_ONE = "SELECT * FROM vnecontact WHERE cid = ?";
	private static final String SQL_EDIT = "UPDATE vnecontact SET fullname = ?, email = ?, subject = ?, content = ? WHERE cid = ?";
	...
	public int add(Contact contact) {
		return getJdbcTemplate().update(SQL_ADD, new Object[] {contact.getFullname(), contact.getEmail(), contact.getSubject(), contact.getContent()});
	}
	...
	public int edit(Contact contact, int cid) {
		return getJdbcTemplate().update(SQL_EDIT, new Object[] {contact.getFullname(), contact.getEmail(), contact.getSubject(), contact.getContent(), cid});
	}
