...
public class UserDAO extends AbstractDAO{
	...
	private static final String SQL_ADD = "INSERT INTO users(username,fullname,remember_token,password,roleId) VALUES(?,?,?,?,?)";
	private static final String SQL_EDIT = "UPDATE users SET username = ?, fullname = ?, password = ? WHERE id= ?";
	...
	public int add(User user,int roleId) {
		return getJdbcTemplate().update(SQL_ADD, new Object[] {user.getUserName(), user.getFullName(), user.getRememberToken(), user.getPassWord(),roleId});
	}
	...
	public int edit(User user, int id) {
		return getJdbcTemplate().update(SQL_EDIT, new Object[] {user.getUserName(), user.getFullName(), user.getPassWord(), id});
	}
