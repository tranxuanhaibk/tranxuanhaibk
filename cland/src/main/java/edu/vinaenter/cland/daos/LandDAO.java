...
public class LandDAO extends AbstractDAO{
	...
	private static final String ADD_LAND = "INSERT INTO lands(lname, description, cid, picture, area, address) VALUES(?,?,?,?,?,?)";
	private static final String DEL_SQL = "DELETE FROM lands WHERE lid = ?";
	private static final String FIND_ONE = "SELECT lid, lname, description, address, date_create AS dateCreate, picture, count_views AS countViews, area FROM lands WHERE lid = ?";
	private static final String FIND_OF_CAT = "SELECT lid, lname, description, address, date_create AS dateCreate, picture, count_views AS countViews, area FROM lands WHERE cid = ?";
	private static final String FIND_ONE_SINGLE = "SELECT lid, lname, description, address,cid, date_create AS dateCreate, picture, count_views AS countViews, area FROM lands WHERE lid = ?";
	private static final String EDIT = "UPDATE lands SET lname = ?, description = ?, cid = ?, picture = ?, area = ?, address = ? WHERE lid = ?";
	private static final String VIEWS = "UPDATE lands SET count_views = count_views + 1 WHERE lid = ?";
	private static final String FIND_RELATED = "SELECT l.*, c.cname FROM lands AS l INNER JOIN categories AS c ON l.cid = c.cid WHERE l.cid= ? && l.lid != ? ORDER BY lid DESC LIMIT ?";
	private static final String COUNT_ITEMS_BY_CAT = "SELECT COUNT(*) AS count FROM lands WHERE cid = ?";
	...
	public int add(Land land) {
		return getJdbcTemplate().update(ADD_LAND, new Object[] {land.getLname(), land.getDescription(),
				land.getCategory().getCid(), land.getPicture(), land.getArea(), land.getAddress()});
		}
	...
	public int edit(Land land) {
		return getJdbcTemplate().update(EDIT, new Object[] {land.getLname(), land.getDescription(), land.getCategory().getCid(), land.getPicture(), land.getArea(), land.getAddress(), land.getLid()});
	}
